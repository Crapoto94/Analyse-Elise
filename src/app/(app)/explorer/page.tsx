...
export default function ExplorerPage() {
  ...
  useEffect(() => {
    ...
    client.getCollections()
      .then(async (res) => {
        ...
        // Fetch counts for each collection in the background
        const countPromises = res.value.map(async (collection: any) => {
          try {
            ...
            console.error(`Error fetching count for ${collection.name}`, e);
          }
        });
  ...
  const handleSync = async () => {
    setSyncing(true);
    try {
      await syncData((p) => setSyncStatus(p));
    } catch (e) {
      console.error(e);
    ...
    <div className="min-h-screen bg-slate-50/50 p-6 md:p-10">
      <header className="max-w-7xl mx-auto flex justify-between items-center mb-10">
        ...
        <div className="flex gap-4 items-center">
          {syncStatus && <span className="text-xs text-slate-400 italic animate-pulse">{syncStatus}</span>}
          <button 
            onClick={handleSync}
            disabled={syncing}
            className={`px-4 py-2 rounded-xl text-white font-bold text-sm shadow-lg shadow-blue-200 transition-all ${syncing ? 'bg-slate-400 rotate-1' : 'bg-blue-600 hover:bg-blue-700 active:scale-95'}`}
          >
            {syncing ? '⌛ Sync...' : '🔄 Synchro Locale'}
          </button>
          <button 
            onClick={() => {
              sessionStorage.removeItem('odata_config');
              router.push('/connect');
            }}
      ...
      {error ? (
        <div className="max-w-7xl mx-auto p-8 rounded-2xl bg-red-50 border border-red-100 text-red-700">
          ...
          <button 
            ...
            className="mt-4 px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all font-medium"
          >
            Réessayer
          </button>
        </div>
