...
async function run() {
  ...
  for (const d of dates) {
    ...
    // On cherche les documents dont la DATE DU PAPIER (DocumentDate) est le samedi, peu importe quand ils ont été enregistrés
    const countDocDate = await getOData(`https://ville-ivry94.illico.city/AppBI/odata/FactDocument/$count?$filter=DocumentDate ge ${start} and DocumentDate lt ${nextDay}`);
    console.log(`${d} (DocumentDate): ${countDocDate} docs`);
  }
}
run();
